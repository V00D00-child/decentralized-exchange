    ...
    describe('order orders', () => {
        ...
        describe('failure', () => {
            it('rejects invalid order ids', async () => {
                ...
                await exchange.cancelOrder(invalidOrderId, { from: user1 }).should.rejectedWith(EVM_REVERT);
            });

            it('rejects unauthorized cancelations', async () => {
                // Try to cancel the order from another user
                await exchange.cancelOrder('1', { from: user2 }).should.rejectedWith(EVM_REVERT);
            });
        });
    });
});